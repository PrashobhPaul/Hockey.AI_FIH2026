...
// The web app reads data relative to its own origin; the Android build ships
// its assets inside the APK, so live data must be fetched from the published
// site absolutely (see src/config.js).
const DATA_BASE = isNative ? `${PAGES_ORIGIN}/data` : `${import.meta.env.BASE_URL}data`
...
async function _sync(force) {
  ...
  // Alerts ride the sync: fresh data in, notifications out. Result alerts for
  // every match first (the tournament-wide feed), then the favourite-team
  // extras. Lazy import keeps the sync path free of Notification-API coupling.
  import('./notify.js').then(n => Promise.resolve(n.checkResultAlerts()).then(() => n.checkFavouriteAlerts())).catch(() => {})
