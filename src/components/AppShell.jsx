...
import AlertsOnboarding from './AlertsOnboarding'
import { useSwipeTabs, SWIPE_PRIORITY } from './useSwipeTabs'
import { isNative, initNative } from '../native'
...
export default function AppShell() {
  ...
  // Android shell: status bar, splash, hardware back, notification-tap
  // routing. A no-op on the web (see src/native.js).
  useEffect(() => { initNative(navigate) }, []) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="min-h-dvh pb-[calc(64px+env(safe-area-inset-bottom))] md:pb-0">
      ...
      <DataBanner />
      {/* Sideloaded APKs don't auto-update; this offers the newer build. */}
      <UpdateBanner />
      ...
      {/* "Install this site" is a browser concept — never inside the app. */}
